import { sampler } from './tone-import.js';
...
export const playTimeLineObj = async (timeLineObj) => {
  const objVals = Object.values(timeLineObj);
  ...
  for (let i = 0; i < objVals.length; i++) {
    ...
    while (index < objVals[i].arrays.length) {
      sampler.triggerAttackRelease(obj.arrays[index], 0.5 * obj.durations[index])
      index++;
    };
